...
- **TestFlight:** build **1.0 (3)** (the card-model rearchitecture) uploaded via API key and processing. Pipeline now scripted end-to-end: archive the `Adaptive Fitness Coach` scheme with the beta toolchain → `xcodebuild -exportArchive` with `destination: upload` + `-authenticationKey{Path,ID,IssuerID}` + `-allowProvisioningUpdates` (creates the distribution cert via the API headlessly, no signed-in account needed). Credentials live in the **git-ignored `.env`** (issuer id, key id, and the *path* to `AuthKey_89VVBRD4AL.p8` — the key material is never committed/read). `DEVELOPMENT_TEAM=7542Q96HNF`, app record `com.memerson.Adaptive-Fitness-Coach`. Bump `CURRENT_PROJECT_VERSION` before each upload.
