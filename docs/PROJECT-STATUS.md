...
_Last updated 2026-07-02 (evening): **P0–P3 implemented.** P1.5 run coaching v2 shipped as TestFlight build 6 (live). **P2 adaptive strength merged to main (sim-verified; NOT yet on TestFlight, on-body validation pending).** **P3 AI coach implemented on branch `p3-ai-coach`** (native trainer conversation via FoundationModels — PCC default; sim-verified end-to-end on the scripted engine; real-model on-device validation pending). Next: P3 on-device validation, then P4 (calorie tracking)._
