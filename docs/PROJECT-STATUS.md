...
_Last updated 2026-07-06 (end of session): **build 18 is IN_BETA_TESTING; FIVE unreleased changes sit on `main` queued for build 19** — from the user's on-device session: (1) reinstall context re-push (`bba0173` — build-18 workaround: force-quit + reopen the phone app after a watch reinstall); (2) watch button "Log it" → "Save for iPhone" (`0de0716`); (3) in-sheet trash for pending review items (`f9478e3`); (4) **"Log a Meal" watch complication** + `LogMealIntent` (`172cc5d` — quick-log sheet now top-level in `SessionContainerView`; watchOS has NO LS scheme opening, so complication taps verify on-device only); (5) **INSIGHTS reserved slot** on run routines with a waiting state before digest history exists (`565bb87` — was invisible until the first build-18 run). **NEXT SESSION (user-declared): adaptive system bugs, details TBD — build 19 ships after those, carrying all five.** The build-18 on-device checklist lives in the "P6 Phase 3 rework" section._

_Previous update 2026-07-06 (later): **the build-17 watch quick-log bug is ROOT-CAUSED and FIXED on `quicklog-always-pending` — see "P6 Phase 3 rework" below.** The live draft/confirm round trips ran the whole lookup ladder synchronously inside the WC reply handler; a locked, backgrounded phone can't finish inside WCSession's reply deadline → minutes of spinner, then the honest fallback. Fix (user-decided): the watch is now **always-pending** — every quick-log goes straight to the guaranteed-delivery review queue; the live channel is deleted from the watch (phone handler kept for build-≤17 compatibility). Background prewarm-on-delivery was designed and deliberately deferred. Merged to `main` + review-hardened same day (see the rework section: pending-transfer persistence, review-card dismiss/timestamp/stroke, honest copy). **Build 18 (P6.1 run insights + this fix) UPLOADED 2026-07-06: processed VALID, `internalBuildState: IN_BETA_TESTING`. NEXT: the rework section's on-device validation checklist** (pull-to-refresh TestFlight if the build doesn't show). **Field bug from the build-18 install (fixed on main, UNRELEASED): a watch-app reinstall drops the OS-held application context and nothing re-pushed it (the only pushes were routine edits + once-per-process activation) → fresh watch stuck at "Syncing…" → false "No session scheduled" while the phone held routines. Fix: `PhoneConnectivityManager.sessionWatchStateDidChange` re-pushes on watch (re)install; workaround on build 18 = force-quit and reopen the phone app.**_
