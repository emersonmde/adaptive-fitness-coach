...
| CQ1b | **Claude-tier billing** (if/when the ladder escalates): consumer OAuth (Pro/Max) is contractually restricted to Claude Code/claude.ai (Anthropic policy, Feb 2026) — a Max plan cannot back this app. "Sign in with Claude" (sanctioned third-party OAuth) bills a separate **prepaid extra-usage pool**, not subscription tokens; `ClaudeForFoundationModels` today supports `.apiKey` / `.proxied` (App-Attest billing to the dev's workspace "coming soon"). | While the app is personal/TestFlight: dev API key, escalation-only + `maxUses` caps → negligible spend, no proxy. Revisit (proxy or App-Attest) only if the app goes public. |
...
| CQ3 | **Where does app-side *search* come from for the PCC path?** Fetch alone can't find the right page ("wendys.com — but which page?"). Validated ladder: (a) **domain-specific free indexes exposed as tools** — USDA FoodData Central (free instant API key; Branded Foods = manufacturer-submitted label data) and Open Food Facts (keyless REST; barcode + text search) as a `search_food_facts` tool — strong for packaged goods, weakest for restaurants; (b) a **dev-owned free-tier web-search key** (e.g. Brave Search API / Google Programmable Search) as a `web_search` tool — a config secret, not user-facing billing, fine at personal scale; (c) **fetch-only navigation** ("browser-use-lite": fetch wendys.com → follow the nutrition link) — works for big chains, but multi-hop quality on Apple's model is exactly what the CQ1 spike measures. | Spike alongside CQ1; expect (a)+(b) as tools on the PCC session, (c) as the fallback behavior, Claude server tools (CQ1 option b) when all three disappoint for an item. |
